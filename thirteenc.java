...
public class thirteenc
{
 	...
 	public static void main(String args[])
 	{
	   ...
	   double S2[][]={{2,10},{10,200}};
	   
	   double mu3[][]={{1.5},{55}};
	    
	   double S3[][]={{3,20},{20,300}};
	   
	   double tao[]={0.5,0.2,0.3};
	   ...
	   while(cnt<100)
	   {
	    ...
	    for(int i=0;i<20;i++)
	    {
	    	for(int j=0;j<3;j++)
	    	{
	    		...
	    		if(j == 0)
	    		{
	    			...
	    			p[j][i]=tao[j]*f(X[i],mu1,S1)/(tao[0]*f(X[i],mu1,S1) + tao[1]*f(X[i],mu2,S2)+tao[2]*f(X[i],mu3,S3));
	    		    System.out.print( "\nP(" + (j) + "," + (i) + ") = " + p[j][i]); 
	    		}
	    		else if(j ==1)
	    		{
	    			
	    			p[j][i]=tao[j]*f(X[i],mu2,S2)/(tao[0]*f(X[i],mu1,S1) + tao[1]*f(X[i],mu2,S2)+tao[2]*f(X[i],mu3,S3));
	    			 System.out.print( "\nP(" + (j) + "," + (i) + ") = " + p[j][i]); 
	    		}
	    		else if(j ==2)
	    		{
	    			
	    			p[j][i]=tao[j]*f(X[i],mu3,S3)/(tao[0]*f(X[i],mu1,S1) + tao[1]*f(X[i],mu2,S2)+tao[2]*f(X[i],mu3,S3));
	    			 System.out.print( "\nP(" + (j) + "," + (i) + ") = " + p[j][i]); 
	    		}
	    ...
	    System.out.print("\n tao[1] =" + tao[0] + " tao[2] =" + tao[1]+ " tao[3] =" + tao[2]);
	    ...
	    for(int j=0;j<3;j++)
	    {
	    	...
	    	{
	    		mu3[0][0]=musum/musum1;
	    		mu3[1][0]=musumd/musum1;
	    	}
	    	//System.out.println(taosum);
	    	musum1=0;
	    	musum=0;
	    ...
	    for(int j=0;j<3;j++)
	    {
	    	...
	    	for(int i=0;i<20;i++)
	    	{
	    		x[0][0]=X[i][0];
	     		...
	     		{
	     			temp[0][0]=x[0][0]-mu3[0][0];
		     		temp[1][0]=x[1][0]-mu3[1][0];
